...
public class ArcBeamController : MonoBehaviour
{
    ...
    [Header("Materiale LineRenderer (assegna un materiale Unlit dal progetto)")]
    ...
    [Header("Z offset diagrammi (negativo = davanti all'arco)")]
    public float diagramZOffset = -0.3f;

    [Header("Y offset appoggi rispetto al piede dell'arco")]
    ...
    [Header("Risoluzione")]
    [Range(50, 200)]
    public int femResolution = 100;

    [Range(100, 500)]
    public int visualizationPoints = 300;
    ...
    void Start()
    {
        ...
        Debug.Log($"[ArcBeamController] ArcStartX={ArcStartX:F2} ArcLength={ArcLength:F2} " +
                  $"horizontalLineY={horizontalLineY:F2} arcPoints={arcPoints?.Length}");
    ...
    Material GetLineMaterial()
    {
        ...
        // Fallback: try URP first, then Built-In
    ...
    void InitializeLineRenderers()
    {
        ...
        if (deformedShapeLine != null)
        {
            ...
            if (mat != null) deformedShapeLine.material = new Material(mat);
            deformedShapeLine.numCornerVertices = 10;
            deformedShapeLine.numCapVertices = 10;
        ...
        if (axialForceLine != null)
        {
            ...
            if (mat != null) axialForceLine.material = new Material(mat);
        ...
        if (shearLine != null)
        {
            ...
            if (mat != null) shearLine.material = new Material(mat);
        ...
        if (momentLine != null)
        {
            ...
            if (mat != null) momentLine.material = new Material(mat);
    ...
    void Update()
    {
        ...
        if (supportPositions.Count >= 2)
        {
            ...
            float fRef = Mathf.Max(loadMagnitude, 1f);
            float mRef = fRef * arcSpan;
    ...
    void RenderDeformedShape(LineRenderer line, ArcBeamData results, float scale)
    {
        ...
        float zPos = arcObject.transform.position.z + diagramZOffset;
    ...
    void SetupInitialScenario()
    {
        ...
        SpawnAtPosition(supportPrefab, leftPoint.x, leftPoint.y + supportYOffset);
        SpawnAtPosition(supportPrefab, rightPoint.x, rightPoint.y + supportYOffset);
    ...
    {
        // diagramZOffset shifts everything (supports, loads, diagrams) in front of the arch mesh
        Vector3 pos = new Vector3(worldX, worldY, arcObject.transform.position.z + diagramZOffset);
        GameObject inst = Instantiate(prefab, pos, Quaternion.identity);

        if (inst.TryGetComponent(out ArcLoad load))
            load.arcController = this;
        else if (inst.TryGetComponent(out ArcDraggableLoad drag))
